...
def generate_image(image_scores, output_dir, dataset, loader, train=False, image_path=None):
    # Get image name
...
def get_metrics(loader, model, loss_function, n_classes, train=True, flip_inference=False, scales=[1], write_images=False, preprocess_mode=None, time_exect=False,  optimizer=None):
    if train:
    ...
    for step in range(samples):  # for every batch
        x, y, mask, path = loader.get_batch(size=1, train=train)
    ...
    return acc_result, miou_result, valid_loss
