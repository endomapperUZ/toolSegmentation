...
def main():
  ...
  for i in range(len(datafiles)):
    ...
    restore_state(model, args.path + '/weights_mininet/endo17/fold1/model_0')
    #else :
    ...
    train(loader=loader, optimizer=optimizer, loss_function=loss_function, model=model, config=CONFIG,
          lr=learning_rate,  name_best_model=args.path+'/weights_mininet/test/'+model_name, evaluation=True, preprocess_mode='normalize',step_num=step_num)
