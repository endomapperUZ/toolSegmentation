...
class LoaderQueue:
    def __init__(self, size, loader, workers, train, val):
        '''
    ...
    def __next__(self):
        ...
        '''
        if len(self.x) > 0:
            return self.x.pop(0), self.y.pop(0), self.mask.pop(0), self.path.pop(0)
        else:
            time.sleep(0.005)
...
def load(name, queue):
    ...
    while True:
        if len(queue.x) < queue.size:
            x_i, y_i, mask_i, path_i = queue.loader.get_sample(train=queue.train, val=queue.val) # Load sample
